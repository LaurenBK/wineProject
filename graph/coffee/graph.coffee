...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      year = sortedYears(curNodesData, curLinksData)
      updateCenters(year)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.variety))
  ...
  # attr is value stored in node, like 'year'
  ...
  # Returns array of year sorted based on
  # current sorting method.
  sortedYears = (nodes,links) ->
    yearr = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.year] ?= 0
        counts[l.source.year] += 1
        counts[l.target.year] ?= 0
        counts[l.target.year] += 1
      # add any missing years that dont have any links
      nodes.forEach (n) ->
        counts[n.year] ?= 0

      # sort based on counts
      year = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      year = year.map (v) -> v.key
    else
      # sort wines by year 
      counts = nodeCounts(nodes, "year")
      year = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      year = year.map (v) -> v.key
  ...
  updateCenters = (year) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(year)
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", 5)  # (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.variety))
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.year)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.variety)).darker().toString()
...
$ ->
  ...
  $("#wine_select").on "change", (e) ->
    wineFile = $(this).val()
    d3.json "data/#{wineFile}", (json) ->
